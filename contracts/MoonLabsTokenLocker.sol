...
contract MoonLabsTokenLocker is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address _mlabToken, address _feeCollector, address referralAddress, address whitelistAddress, address routerAddress) public initializer {
    __Ownable_init();
   ...
   */
  function createLockMLAB(address tokenAddress, LockParams[] calldata locks) external {
    ...
    emit LockCreated(msg.sender, tokenAddress, uint64(locks.length), nonce - 1);
  }
  ...
  function createLockPercent(address tokenAddress, LockParams[] calldata locks) external {
    ...
    /// Transfer tokens to contract and get amount sent
    uint amountSent = _transferAndCalculateWithFee(tokenAddress, totalDeposited, tokenFee);
  ...
  function withdrawUnlockedTokens(uint64 _nonce, uint amount) external {
    ...
    _transferTokensTo(lockInstance[_nonce].tokenAddress, msg.sender, amount);

    /// Delete lock instance if current amount reaches zero
    if (lockInstance[_nonce].currentAmount <= 0) _deleteLockInstance(_nonce);
   ...
   */
  function transferLockOwnership(uint64 _nonce, address _owner, address _withdrawer) external {
   ...
   */
  function relockMLAB(uint64 _nonce, uint amount, uint64 startTime, uint64 endTime) external payable {
   ...
   */
  function splitLockMLAB(address to, address withdrawalAddress, uint64 _nonce, uint amount) external {
    ...
    _splitLock(_nonce, depositAmount, currentAmount, amount, tokenAddress, withdrawalAddress, to);
  }
  ...
  function splitLockPercent(address to, address withdrawalAddress, uint64 _nonce, uint amount) external {
    ...
    if (!whitelistContract.getIsWhitelisted(tokenAddress)) {
      ...
      uint tokenFee = MathUpgradeable.mulDiv(currentAmount, percentSplitPrice, 10000);
      /// Deduct fee from token balance
      lockInstance[_nonce].currentAmount -= tokenFee;
      lockInstance[_nonce].depositAmount -= tokenFee;
      /// Transfer token fees to the collector address
  ...
  function getMLABFee(uint amountInEth) public view returns (uint) {
    ...
    uint[] memory amountOuts = routerContract.getAmountsOut(amountInEth, path);
    return MathUpgradeable.mulDiv(amountOuts[1], (100 - mlabDiscountPercent), 100);
  }
   ...
   */
  function _relock(uint64 _nonce, uint amount, address tokenAddress, uint64 startTime, uint64 endTime) private {
    /// Check that sender is the lock owner
    require(lockInstance[_nonce].ownerAddress == msg.sender, "Ownership");
    ...
    if (amount > 0) {
      ...
      require(IERC20Upgradeable(tokenAddress).balanceOf(msg.sender) >= amount, "Token balance");
      /// Transfer tokens to contract and get amount sent
   ...
   */
  function _splitLock(uint64 _nonce, uint depositAmount, uint currentAmount, uint amount, address tokenAddress, address withdrawalAddress, address to) private {
    /// Check that sender is the lock owner
    require(lockInstance[_nonce].ownerAddress == msg.sender, "Ownership");
    ...
    uint newDepositAmount = MathUpgradeable.mulDiv(depositAmount, amount, currentAmount);
    ...
    lockInstance[nonce] = LockInstance(tokenAddress, to, withdrawalAddress, newDepositAmount, amount, lockInstance[_nonce].startDate, lockInstance[_nonce].endDate);
   ...
   *    endDate Date when all tokens are fully unlocked
   */
  function _createLocks(address tokenAddress, LockParams[] calldata locks, uint amountSent, uint totalDeposited) private {
    uint64 _nonce = nonce;
    for (uint64 i = 0; i < locks.length; i++) {
      ...
      /// Create a new Lock Instance and map to nonce
      lockInstance[_nonce] = LockInstance(tokenAddress, locks[i].ownerAddress, locks[i].withdrawalAddress, MathUpgradeable.mulDiv(amountSent, depositAmount, totalDeposited), MathUpgradeable.mulDiv(amountSent, depositAmount, totalDeposited), startDate, endDate);
      /// Map token address to nonce
   ...
   */
  function _calculateTotalDeposited(LockParams[] memory locks) private pure returns (uint) {
    uint totalDeposited;
   ...
   */
  function _transferAndCalculate(address tokenAddress, uint totalDeposited) private returns (uint) {
   ...
   * @return total amount sent
   */
  function _transferAndCalculateWithFee(address tokenAddress, uint totalDeposited, uint tokenFee) private returns (uint) {
    /// Get balance before sending tokens
    uint previousBal = IERC20Upgradeable(tokenAddress).balanceOf(address(this));

    /// Transfer tokens from sender to contract
    _transferTokensFrom(tokenAddress, msg.sender, totalDeposited + tokenFee);
   ...
   */
  function _transferTokensFrom(address tokenAddress, address from, uint amount) private {
   ...
   * @param amount number of tokens being transferred
   */
  function _transferTokensTo(address tokenAddress, address to, uint amount) private {
  ...
  function _handleBurns(uint value) private {
    ...
    if (burnMeter >= burnThreshold) {
      ...
      path[0] = routerContract.WETH();
      path[1] = address(mlabToken);
      uint[] memory amounts = routerContract.swapExactETHForTokens{ value: burnMeter }(0, path, 0x000000000000000000000000000000000000dEaD, block.timestamp);
      /// Reset burn meter
      burnMeter = 0;
   ...
   */
  function _distributeCommission(string memory code, uint commission) private nonReentrant {
   ...
   */
  function _calculateLinearWithdraw(uint64 _nonce) private view returns (uint) {
    uint currentAmount = lockInstance[_nonce].currentAmount;
    uint depositAmount = lockInstance[_nonce].depositAmount;
    uint64 endDate = lockInstance[_nonce].endDate;
    uint64 startDate = lockInstance[_nonce].startDate;
