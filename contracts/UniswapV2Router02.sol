...
contract UniswapV2Router02 is IUniswapV2Router02 {
  ...
  function custompair(address tokenA, address tokenB) external view returns (address pair) {
  ...
  function swapExactETHForTokensSupportingFeeOnTransferTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external payable virtual override ensure(deadline) {
    require(path[0] == WETH, "UniswapV2Router: INVALID_PATH");
...
library UniswapV2Library {
  ...
  function custompair(address factory, address tokenA, address tokenB) external pure returns (address pair) {
