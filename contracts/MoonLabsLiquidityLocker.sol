...
contract MoonLabsLiquidityLocker is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   */
  function createLockMLAB(address tokenAddress, LockParams calldata lock) external payable {
  ...
  function createLockPercent(address tokenAddress, LockParams calldata lock) external {
    ...
    /// Transfer tokens to contract and get amount sent
    uint amountSent = _transferAndCalculateWithFee(tokenAddress, depositAmount, tokenFee);
  ...
  function nukeLock(uint64 _nonce) external {
    ...
    _transferTokensTo(lockInstance[_nonce].tokenAddress, address(0x000000000000000000000000000000000000dEaD), currentAmount);
   ...
   */
  function relockMLAB(uint64 _nonce, uint amount, uint64 unlockDate) external payable {
   ...
   * @param unlockDate time in seconds to add to the existing end date
   */
  function relockETH(uint64 _nonce, uint amount, uint64 unlockDate) external payable {
    address tokenAddress = lockInstance[_nonce].tokenAddress;
   ...
   * @param unlockDate time in seconds to add to the existing end date
   */
  function relockPercent(uint64 _nonce, uint amount, uint64 unlockDate) external {
    address tokenAddress = lockInstance[_nonce].tokenAddress;
   ...
   * @param amount number of tokens sent to new lock
   */
  function splitLockMLAB(address to, uint64 _nonce, uint amount) external payable {
    uint currentAmount = lockInstance[_nonce].currentAmount;
    ...
    _splitLock(_nonce, depositAmount, currentAmount, amount, tokenAddress, to);
  }
   ...
   */
  function _relock(uint64 _nonce, uint amount, address tokenAddress, uint64 unlockDate) private {
    /// Check that sender is the lock owner
    require(lockInstance[_nonce].ownerAddress == msg.sender, "Ownership");
    /// Check for end date upper bounds
    require(unlockDate + lockInstance[_nonce].unlockDate < 10000000000, "End date");
   ...
   */
  function _splitLock(uint64 _nonce, uint depositAmount, uint currentAmount, uint amount, address tokenAddress, address to) private {
    /// Check that sender is the lock owner
    require(lockInstance[_nonce].ownerAddress == msg.sender, "Ownership");
    ...
    lockInstance[nonce] = LockInstance(tokenAddress, to, newDepositAmount, amount, lockInstance[_nonce].unlockDate);
   ...
   *    unlockDate Date when all tokens are fully unlocked
   */
  function _createLock(address tokenAddress, LockParams calldata lock, uint amountSent) private {
   ...
   */
  function _transferAndCalculate(address tokenAddress, uint amount) private returns (uint) {
   ...
   * @return total amount sent
   */
  function _transferAndCalculateWithFee(address tokenAddress, uint amount, uint tokenFee) private returns (uint) {
    /// Get balance before sending tokens
    uint previousBal = IERC20Upgradeable(tokenAddress).balanceOf(address(this));
