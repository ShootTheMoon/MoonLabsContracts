    ...
    OwnableUpgradeable
{
     ...
     */
    function purchaseWhitelistWithCode(
        address _address,
        string calldata code
    ) external {
        /// Check if token is already whitelisted
    ...
    function getMLABFee(uint amountInUSD) public view returns (uint) {
        ...
        uint[] memory amountOutsUSD = routerContract.getAmountsOut(
            amountInUSD,
            pathUSD
        );
        ///  Get price quote via uniswap router
        ...
        uint[] memory amountOutsMLAB = routerContract.getAmountsOut(
            amountOutsUSD[1],
            pathMLAB
        );
        return
    ...
    function _distributeCommission(
        ...
        usdContract.safeTransfer(
            referralContract.getAddressByCode(code),
            commission
        );
    ...
    function _checkIfValidPair(address _address) private view returns (bool) {
        if (tokenToWhitelist[_address] && !pairToBlacklist[_address])
            return true;
        return false;
    }
}
