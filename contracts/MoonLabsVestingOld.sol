...
contract MoonLabsVestingOld is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  */
  ...
  /**
   * @notice Determine if given address is withdraw owner
   * @param _address address of potential owner
   * @param _nonce ID of desired lock instance
   * @return bool depending on ownership
   */
  function isWithdrawOwner(address _address, uint64 _nonce) public view returns (bool) {
    /// Check that caller is the withdraw owner of the lock
    uint64[] memory withdrawArray = withdrawToLock[_address];
    for (uint64 i = 0; i < withdrawArray.length; i++) {
      if (withdrawArray[i] == _nonce) {
        return true;
      }
